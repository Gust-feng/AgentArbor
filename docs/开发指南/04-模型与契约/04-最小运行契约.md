...
当前正式 Ordinary 闭环由 `OrdinaryAgentFeature` 通过 `ordinary-run/v2` snapshot 和 `canonicalMessages` 保存运行与跨轮模型事实；旧 RuntimeDatabase 不是 Ordinary 的恢复来源。Multi-Agent 继续拥有独立 Deep store 和 run-tree 契约，两类状态不互转。
