...
- 首屏必须是 Trae/Cursor 式桌面工作会话：左侧项目 / 会话导航，中间是持续工作会话流，右侧是轻量 `待办 / 上下文 / 证据 / 近期活动`，底部是常驻任务输入框。默认空态主标题为 `要完成什么？`，首屏只展示任务、上下文入口、轻量活动、判断、证据、待确认、结果和下一步。
- 工作台必须支持随意问题。普通问答、闲聊、模型身份问题、小解释、小翻译或不需要工作区探索的请求，应展示一条直接回答；不得强行派生 child agent、生成报告、显示空 artifact 或把问题包装成项目分析任务。
...
- 左侧导航只表达用户工作空间：`新建任务`、技能、自动化、任务列表、最近任务、工作区状态。不得把 `土壤`、`方向智能`、`执行智能`、rootlet 或运行时层级当作产品导航。
...
- 右侧上下文栏是轻量工作辅助，默认常驻但只展示 `待办`、`上下文 / 材料`、`证据` 和 `近期活动`。它不是卡片墙，不展示成果大预览，不展示架构监控。诊断、Agents、设置进入默认隐藏的开发者详情抽屉，默认不抢占首屏。
...
- 任务输入可以携带上下文引用、权限引用和只读短预览，但这些高级输入默认折叠；UI 只能收集引用、短摘要和短 preview，不读取或展示未授权文件正文。旧 goal-only 输入必须继续兼容。
...
- UI 视觉应偏成熟桌面工作台：安静、克制、可扫读，避免深色大块标题、装饰性 hero 和卡片堆叠。宽屏下不重叠、不产生横向滚动，移动窄屏下改为单列。右侧上下文栏应轻，不得变成多卡片监控台；开发者详情抽屉默认关闭，失败时可以提示打开诊断，但 Agents、设置和折叠调试区不得挤占主画布。
...
- Desktop run canvas 若包含 `chat.answer` 或 `workSession.directAnswer`，主 transcript 的助手回合应直接显示该回答；`workSession.report` / `workSession.artifact` 缺失时不得补假结果块。普通问题优先走 `desktop_chat`，不得为了显示流程而进入 Work Session。
