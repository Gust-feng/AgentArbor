...
删除普通会话后，Host 通过 Conversation coordinator 解除其直接 owner（Space 或 Workspace）的链接；该动作只删除 Conversation 及其运行资产，不删除 owner 本身、Space 软件资产或外部真实文件夹。删除 Space 时，协调器先停止直属对话进程，再删除其 Conversation/Run、owner-scoped Agent Notes 与路径依赖、清理 Personal Knowledge 来源关联，最后删除 Space 及其软件资产；外部 Workspace 引用只解除，不删除外部源。删除 Workspace 时，协调器先停止直属对话进程，再删除 Workspace owner 的 Conversation/Run、owner-scoped Agent Notes 与路径依赖、解除其全部 Space links，最后删除 Workspace 记录；外部真实文件夹和知识库副本始终保留。
...
- Ordinary 使用 `ordinary-run/v7` snapshot：每个 run 原子写入 `runtime/ordinary-agent/runs/<runId>/snapshot.json`，列表 manifest 只是可重建索引；conversation control 使用独立的 `ordinary-conversation/v3` 文档。启动枚举或首次读取 `ordinary-run/v5` 时，仓储会先把原始字节保留为同目录 `snapshot.ordinary-run-v5.json`，再原子迁移为 v7；迁移不增加业务 revision，备份暂不自动删除。v6 的路径键 Agent Notes 版本无法可靠映射到稳定 owner，明确视为不兼容，不由当前 cwd 或路径猜测补全。旧 RuntimeDatabase、raw `run.json`、sidecar event/tool-call 和 v4 及更早 schema 仍不读取、不迁移、不双写
...
- 路径依赖（程序性记忆）已由 `PathDependencyFeature` 落地，owner 与 Agent Notes 一样只使用 `global`、`spaceId` 或 `workspaceId` 的稳定身份。Run birth 只注入当前冻结作用域内有预算的记忆目录，绝不全量注入方法论正文；模型可按需使用 `MemorySearch` 发现、`MemoryRead` 完整读取、`MemoryReference` 明确记录采用，以及 `PathDependencySave` 创建或以 revision 更新。模型运行工具只能使用 global + 当前 frozen direct owner。读取和采用事实归 Ordinary，PathDependencyFeature 不建立平行 run 或 read-model。Panel 的独立 Memory Center 可显式选择已登记且未删除的 Space / Workspace，并经 feature facade 校验后管理全局与所选 owner 的路径依赖、展示来源、版本及读取/使用投影；它不接受任意未验证 ownerId。
