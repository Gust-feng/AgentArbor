...
- 首屏必须是运行界面：目标输入、AI mode、配置状态、运行按钮、运行总览、工作流阶段时间线、Rootlet 工作区、模型调用追踪、模型输出、Agent Transcript、收束解释和方向包结果；EventLog 与 Observation Snapshot 只能作为辅助 / 调试视图，不能再作为主内容大块压在页面上。
