...
- 历史运行和恢复运行的 read-model 只使用 run 创建时冻结的 `capabilitySnapshot`、`informationAccess`、`basicRun` 与 append-only `basicEvents`；缺少出生事实或明确终态事件的开发期旧 Ordinary snapshot 直接失效，不从当前配置、model call、confirmation 或 UI 状态回填
...
- 通用 ToolCenter/kernel 只读取工具输出顶层 `continuation / continuations`。MCP adapter 只把 `structuredContent` 直属且含可执行 `nextInput` 的同名对象提升为顶层 canonical continuation；这是 AgentArbor 的一层适配约定，不是 MCP 标准 continuation，也不会把 ref-only 业务字段或更深对象误判为续读。小结果完整返回且不附带从头重读入口；真实分页/offset ceiling 或副作用请求无法安全重放时，必须返回明确失败事实，不得返回 `completed + truncated` 的不可续读结果，也不得生成会重放 POST/PUT/DELETE 的 synthetic continuation
- 序列化结果超过当前 180,000 字符内联边界时，ToolCenter 使用每个 PanelRuntime 唯一、Host-owned 的进程内 `ToolOutputStore` 保存当前完整文本或序列化 JSON，即使 producer 已提供下一页 continuation 也不能跳过：原 producer continuation 保留在已保存的当前结果里，模型读完当前页后再获得它，避免只剩 preview 和下一页 cursor。结果返回 4,000 字符预览、opaque `tool-output://` 引用和 `read_tool_output` 的下一段输入，并保留带外附件；显式 failed/cancelled 的超大 output、error 与 errorFacts 作为一份完整失败证据保存。父 run 必须冻结并授权 reader，Deep child / Sub-Agent 在真实 broker 也具备 reader 时把它作为 transport companion 自动继承，不扩张其他业务工具。读取不会重新执行原工具；`read_tool_output` 以 UTF-16 code-unit offset 每次最多读取 30,000 字符，窗口不得拆分 surrogate pair，最后一段读完即释放该 ref。默认 store TTL 最长 24 小时、最多 128 项、单项最多 4,000,000 字符、总计最多 32,000,000 字符；Ordinary run 终态释放本 trace 尚未消费的引用；Deep run 为 post-terminal child continuation 保留稳定 owner，在删除所属 Multi-Agent conversation 时回收；Panel 关闭全量清理。引用也会在过期或进程重启后失效，不可枚举、不写入 RuntimeDatabase。超过单项/总量/条目容量时不创建引用并返回明确 delivery failure；原 failed/cancelled/approval 状态、错误域、错误码和确认请求必须保留，只有原 completed 结果因无法完整交付而转为 failed
- MCP adapter 只保留服务端原生 `content[]` 与可选 `structuredContent`，不生成 `summary / mcpResult / result` 多份包装；只对“text 可解析 JSON 且与 structuredContent 深度完全相等”的精确镜像做无损去重，其他内容完整保留；`isError=true` 成为正式工具失败。图片、音频和非图片 embedded resource blob 分别转成带外 `image / audio / file` 类型的 `ModelInputAttachment`，JSON 只保留 MIME、文件名/URI、byteLength 和附件索引，每个 MCP 媒体附件当前上限为 20 MiB。OpenAI-compatible Chat Completions 只把内联 wav/mp3 音频映射为 `input_audio`；Chat Completions 与 Responses 的内联 file 都按官方契约发送 `data:<mime>;base64,<data>`；OpenAI Responses 当前明确拒绝音频输入；其他无法由当前 provider 协议消费的媒体同样必须形成可观察失败，不能伪装成普通 file 或静默丢弃
