...
- `pnpm test` 必须覆盖 panel config API、no-AI run、fake AI run、openai-compatible 缺 key / 缺 model、async run job、running/completed polling、HTTP JSON 脱敏、provider fetch 未调用、默认中文 UI、运行中骨架提示、工作流阶段时间线、Rootlet 工作区、模型调用追踪、Agent Transcript、收束解释、方向包结果和运行追踪投影。
...
- 本地 panel 的运行追踪必须展示 phase / stage / status、等待点、工作流阶段时间线、rootlet kind 状态、按 kind 的模型计数、按 kind 的候选计数、AI candidate / fallback、模型事件序列、Agent Transcript、收束结果、方向包校验和 provider 配置状态。
