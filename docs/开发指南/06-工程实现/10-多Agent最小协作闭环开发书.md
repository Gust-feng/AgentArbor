...
- child 不设置工程默认轮次预算，也不在 child 模型请求上注入固定输出/延迟预算；只有父 Agent 在 `DeepChildSpec.maxModelRounds / maxToolRounds` 中显式给出时才限轮次。省略预算表示不设固定轮次上限，由 child 自主工作到自然完成、等待确认、停止或外部取消。
...
- `src/app/panel-ui-app-structure.test.ts`
- `src/app/panel-ui-runtime-structure.test.ts`
...
node --test dist/app/panel-ui-app-structure.test.js dist/app/panel-ui-runtime-structure.test.js
pnpm build:panel
git diff --check
```
