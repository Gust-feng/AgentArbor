...
本章定义代码实现阶段的工程约束。当前仓库已进入 Desktop Basic Agent Runtime + Local Runtime Lite Profile 实现：Desktop Shell、AgentTurnRuntime、ToolCenter、本地策略沙盒、Confirmation Gate、RunEvent 安全投影、Skills 最小闭环和轻量 RuntimeDatabase 共同支撑当前桌面 Agent 工作流。运行记录可以进入 appHome / runHome 下的 RuntimeDatabase 安全投影；完整数据库后端、迁移体系、治理回流、Routines、可见 deep 入口、deep 后端扩展和 Full Profile 能力仍按 ADR-0022 / ADR-0023 / ADR-0024 的共享契约等待未来显式启动。
