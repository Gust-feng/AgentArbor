...
- Desktop 首选入口默认进入 `desktop_chat`。普通问题、随意提问和连续追问都先按桌面助手 Root Agent 处理；模型可以在授权范围内通过 AgentTurnRuntime 调用 `search` / `read` 工具。复杂方向成形任务必须由用户显式选择 `runMode = "deep"` 后进入 Underground Cognitive Runtime；当前深度模式只开发到地下组织 / Plan 边界，不进入 Aboveground 执行。
...
- Desktop Work Session purpose 覆盖 `work_session_decision`、`work_session_child_material`、`work_session_synthesis` 和 `work_session_direct_answer`。它们是历史兼容 / 实验路径，不是当前 Desktop 产品主线。当前显式深度模式应走 Underground Cognitive Runtime 的 intent / growth / rootlet / convergence / handoff_narrative contracts，并停在 Plan 边界。
...
- Desktop 入口不得把所有输入强行升级为复杂任务，也不得用工程关键词替模型判断。`runMode = "agent"` 是默认路径，经 AgentTurnRuntime 调用 `desktop_chat`，允许模型在授权范围内使用 `search` / `read` 后返回自然语言回答或建议用户切换深度模式；`runMode = "deep"` 是用户显式入口，只运行 Underground Cognitive Runtime 的地下组织、父层综合和收束，当前不进入 Aboveground 执行。
...
- Desktop 显式深度模式：fake/stub AI 只有在请求体携带 `runMode = "deep"` 时才进入 Underground Cognitive Runtime；项目分析、文件阅读或报告任务不得由工程关键词自动升级。
