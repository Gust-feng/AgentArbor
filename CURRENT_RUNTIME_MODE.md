...
Space/Workspace/Conversation 资源权限的目标事实源是 [ADR-0035](docs/架构设计/产品架构/ADR-0035-Conversation双资源owner与统一运行作用域.md)、[ADR-0034](docs/架构设计/产品架构/ADR-0034-Space工作区引用与对话资源生命周期.md) 和 [工程指南](docs/开发指南/06-工程实现/18-Space工作区对话与资源权限开发指南.md)。Conversation 只有一个 owner（Space 或 Workspace），创建后冻结不可切换；Run 出生前由 Host 解析统一运行作用域（cwd、managedRoot、workspaceGrants、confirmationPolicy）。生产提交入口要求 owner，run cwd、Skills、Sub-Agent roots 与文件工具边界统一来自该 owner 的 execution root；Agent Notes 与其他长期记忆则使用同一个冻结 owner 身份，绝不从路径推导，不再读取全局默认目录。ADR-0034 的机械边界继续有效：Space 维护自己的软件资产并引用外部 Workspace，模型看到真实路径，后端保留 `referenceId` 和首次添加时捕获的文件系统来源身份，普通 Shell 每次确认，`full_access` 同时覆盖 Shell 和文件路径。来源身份只用于执行授权，不进入模型正文。系统不轮询或后台扫描外部 Workspace；只有打开、预览、读取、修改、附件交付或文件工具实际解析路径时才检查引用根，确认根消失、类型改变或同一路径已换成另一文件系统对象后立即移除当前引用，重新添加才产生新身份。本文其余段落中的 `workspaceRoot` 与 Task Soil 表述都按这一口径理解，不存在第二套并行口径。

删除普通会话后，Host 通过 Conversation coordinator 解除其直接 owner（Space 或 Workspace）的链接；该动作只删除 Conversation 及其运行资产，不删除 owner 本身、Space 软件资产或外部真实文件夹。删除 Space 时，协调器先停止直属对话进程，再删除其 Conversation/Run、owner-scoped Agent Notes 与路径依赖、清理 Personal Knowledge 来源关联，最后删除 Space 及其软件资产；外部 Workspace 引用只解除，不删除外部源。删除 Workspace 时，协调器先把 Workspace 持久化标记为 `deleting` 并拒绝新的 owner 进入，再停止直属对话进程，删除 Workspace owner 的 Conversation/Run、owner-scoped Agent Notes 与路径依赖，解除其全部 Space links。当前 Workspace 记录不会在级联失败时回滚或物理移除；失败会保留 `deleting` 屏障，进程重启后继续拒绝进入，用户可通过显式删除重试收口。当前尚未实现按 phase journal 自动恢复；外部真实文件夹和知识库副本始终保留。

> Workspace 删除的实现补充：级联成功后由 Host 调用 `purgeWorkspace` 物理清除 Workspace 软件侧登记；级联失败或重启恢复时保留 `deleting` 屏障，必须显式重试，不能在没有 phase journal 的情况下直接清理登记。
...
后端只有一个生产组合根：`createPanelRuntime()`。它当前创建 `OrdinaryAgentFeature`、`AgentNotesFeature`、`PathDependencyFeature`、`SpaceFeature` 以及所需的中性资源；`ordinary-routes` 只做 HTTP/SSE 适配。路径依赖由独立 feature 持久化和查询，运行中读取与明确采用的事实仍归 Ordinary。PathMemory / ExperienceCandidate 是保留源码的历史实验，生产组合根不创建它们，也不采集运行档案。Multi-Agent 的 feature、store、control registry 和资源装配不进入当前生产运行时，也不参与关闭流程。

`SpaceFeature` 是独立的内容组织功能：它持有 Space 元数据、软件资产和外部 Workspace 引用。`workspace_folder` 是用户文件系统中的只读权限引用；同一 Workspace 可被多个 Space 独立引用，同一 Space 内禁止重复或父子嵌套引用。Workspace 没有跨 Space 移动操作，目标 Space 需要使用时重新添加；移除引用只撤销当前 Space 权限，不删除外部源内容。`managed_folder`、Workbench 文档、生成产物等是 AgentArbor 自己维护的软件资产，用户和模型可通过结构化工具编辑、重命名和移动；删除 Space 或软件资产时才物理删除这些资产并经过删除确认。Conversation 固定属于一个直接 owner（Space 或 Workspace），不能作为普通引用添加、移动或解绑，只能删除。外部路径型引用在添加时保存平台文件身份（当前实现为 `dev + inode`），并通过引用事件与权限事实协调；软件自有资产不使用这项外部解绑规则。Space 列表和树查询不探测磁盘，真实的打开、预览、正文操作、附件工具、视觉附件交付和标准文件工具访问才校验引用根。确认目录或文件根消失、类型变化或来源身份不符后，系统删除当前 Space 的外部引用、撤销活动 Run 并收口关联进程；不显示可恢复的 `unavailable` 节点，原路径恢复也不会恢复旧身份，重新添加才生成新 `referenceId`。目录下的文件和子目录由真实文件系统扫描、读取和修改，不作为逐项数据库记录。Host 在每轮提交时从 Conversation owner 解析其 owner 资源：Space 解析软件资产与 Workspace 引用，Workspace 解析当前 mount；并将它们冻结为本轮资源与精确路径授权。后续新增引用只影响后续轮次，撤销引用通过 live deny overlay 立即阻止活动 Run 的后续访问。模型继续使用标准 `Read / Glob / Grep / Write / Edit` 和真实绝对路径；Host 向这些成熟工具注入 Run-scoped path authorization，后端保留 `referenceId` 作为权限、进程关联和审计事实，不再注册 `SpaceWrite / SpaceEdit` 第二套文件 API。普通模式的文件工具只能命中本轮冻结且当前未撤销的引用，`full_access` 可访问引用集合外的真实路径，但仍不能访问已撤销引用。Agent 通过结构化空间工具管理引用与空间内容：`SpaceMountLocalPath` 把用户本轮明确给出、且经用户确认的绝对路径挂载为 `local_file` / `workspace_folder` 引用（必须走确认门，工具绝不猜测或编造路径；挂载后下一轮 run 起文件工具才可读写该路径）；`SpaceCreateManagedFolder` 在 Host 托管根下创建软件自有文件夹；`SpaceCreateEntry / SpaceRenameEntry / SpaceDeleteEntry` 管理 folder 引用根内条目（删除条目走确认门）；`SpaceUpdateCaption` 更新引用图片说明。`generated_artifact` 不作为 Agent 可见引用类型暴露（面板注册能力保留）；`SpaceAddReference` 只接受本轮附件（`local_attachment`）与网页引用。空空间的 Space-owned run 仍暴露 `AttachmentList`，让模型能发现"本轮无附件"，避免工具族整体消失。结构化 Space 工具的创建、添加、重命名、移动和引用移除默认执行；挂载、删除 Space、Conversation 或软件资产进入确认门。Panel 只消费 query projection。

Workbench 的结构化业务数据保存在 `runtime/workbench.sqlite3`。`SpaceFeature` 独立拥有 `spaces / space_references` 表；WorkspaceFeature 拥有 Workspace 与 Space link 记录；Host 另有 `space_conversation_links` 和 `space_conversation_deletions` 表，只保存跨 Space/Ordinary 工作流的身份和检查点，不复制 feature 正文或状态。前者协调新 Conversation 的 owner link 与单独删除：创建按 `prepared -> owner_linked -> conversation_created`，删除按 `prepared -> processes_stopped -> conversation_deleted -> reference_unlinked`；恢复只收尾已有事实，链接身份变化时 fail closed。后者协调 Space 级联删除，按 `prepared -> processes_stopped -> conversations_deleted -> space_deleted` 推进，失败保存 `resumeFrom`。Workspace 删除由单独 coordinator 在同一进程串行完成直属 Conversation/Run、owner memory 和 Space link 的清理；WorkspaceFeature 当前保留记录并持久化 `deleting` 状态，失败不会回滚删除屏障，显式 DELETE 可重试，尚未实现独立 phase journal 的自动启动恢复。不把外部真实文件夹纳入删除目标。Panel 在监听端口前恢复所有 Space journal；删除中的 Space、Workspace 或 Conversation 拒绝新的提交。进程无法确认停止时记录 `stop_pending` 并阻止继续删除。

`PersonalKnowledgeFeature` 独立拥有个人 Markdown 笔记、知识收藏与链接、主题归属和最近打开事实。收藏 Space 文件或文件夹时，Host 把所选内容复制到 `runtime/knowledge-assets/`，知识页面和物理副本从此属于 Personal Knowledge；`sourceReferenceId/sourceRelativePath` 只记录可删除的来源关联。删除 Space 会删除该 Space 的笔记并清除知识副本的来源关联，但保留知识页面和物理副本；只有取消收藏才由 Personal Knowledge 删除它们。启动时对账的只是软件自有知识目录与知识页面，不扫描外部 Workspace。各 owner 只共享 Host 持有的 SQLite 连接，不共享业务仓储或状态。全新数据目录首次启动时，Host 会创建"我的空间"与"学习空间"并导入内置初始数据；文件夹通过与用户相同的 `managed_folder` 生命周期创建，Markdown、代码、PDF 与图片作为真实文件复制到 `runtime/space-folders/`，网页使用普通 `web_page` 引用，不创建 Conversation、Run、活动或时间线。需要进入知识库的文件随后通过正式 `collectSpaceReference` 命令复制到 `runtime/knowledge-assets/`，再按普通主题和链接契约组织；初始化不再向 `workbench_assets` 写入专用内容，也不创建遗留 `material` 页面。已初始化过的数据目录（升级或复用）只补齐"我的空间"容器，绝不重新导入内置内容，也不覆盖已有文件；用户编辑或删除后不会自动恢复。初始化通过“首次安装资格 + 导入完成”两个 SQLite 事实保持幂等；首次导入中断后只补齐缺失文件和关系，不覆盖已存在文件。仅清空知识内容、Space 或部分 runtime 子目录不等同首次安装，当前没有自动原地恢复出厂数据的入口。设置页始终通过内置 provider preset 展示 OpenAI、DeepSeek、Kimi/Moonshot、GLM 与 MiniMax，但 preset 不是已配置模型 profile；只有用户保存 API Key、启用 profile 且存在模型时，该模型才进入会话模型选择器。模型图标只由模型/厂商身份决定，不依赖配置交互时序。Ordinary run、conversation、Pi Session 和 ToolOutputStore 仍使用各自现有文件存储，不进入 Workbench 数据库。当前开发数据允许直接重建：Space 写入 `space-tree/v5`，SQLite 迁移会清除没有来源身份的旧外部引用，文件仓储不兼容读取 `space-tree/v3`；旧路径必须由用户重新添加，不能在缺少身份事实时猜测恢复权限。

> Workspace 级联失败不会被备份或恢复流程隐式吞掉；`deleting` 屏障和显式重试仍是当前可观察事实。

Workbench 在线备份只有在 runtimeHome 路径租约内确认引用物理删除 journal、Conversation link journal 和 Space/Conversation 级联删除 journal 都为空时才会读取 SQLite 与软件自管目录；存在损坏或未收口 journal 时明确拒绝快照，不能产出跨时点备份。用户选择的恢复包通过校验后，当前 Panel 会先停止接受新请求、释放 Ordinary，再按 Personal Knowledge、Space 的依赖顺序等待队列和删除恢复收敛，然后制作安全备份和写入 pending restore；当前实例此后只允许关闭并重启。pending 软件自管目录先发布并刷盘，pending SQLite 最后发布并作为完整恢复包的提交点。若升级前遗留了“未应用 pending restore + 非空引用物理删除 journal”，下次服务器启动会在 `runtimeHome` 单写者 lease 内先用旧 SQLite 装配 recovery-only Space 边界，收口 journal 并关闭旧连接，再应用 pending restore；journal 损坏、引用身份变化或已进入 Workbench restore journal/commit 后仍存在的跨世代含混状态继续 fail closed。外部 `local_file` 的删除暂存内容不属于 Workbench 备份资产。

Personal Knowledge 的标题与 Markdown 正文由 SQLite FTS5 索引，并保留中文子串补全查询；Panel 搜索和 Ordinary Agent 的 `KnowledgeSearch` 使用同一 feature query。Ordinary 还通过冻结工具边界使用 `KnowledgeRead / KnowledgeCreateNote / KnowledgeUpdateNote / KnowledgeDeleteNote / KnowledgeCollect / KnowledgeList / KnowledgeReadPage / KnowledgeNoteHistory / KnowledgeRestoreNote / KnowledgeUpdateAssetText / KnowledgeUncollect / KnowledgeCreateTheme / KnowledgeAssignTheme / KnowledgeUnassignTheme`，所有写入仍由 `PersonalKnowledgeFeature` command 与 revision 契约执行，模型和 ToolCenter 不直接访问数据库。笔记历史以 append-only `personal_note_revisions` 保存每个版本的全量快照（含 base_revision 父链、operation、actor 与 change_summary）；Agent 通过 `KnowledgeNoteHistory` 读取历史、`KnowledgeRestoreNote` 把笔记完整内容恢复到某个历史版本并写出新 revision（不重写历史），陈旧 `expectedRevision` 或缺失目标版本一律拒绝；回溯能力只暴露给 Agent 工具，用户视图仍只有当前版本。知识库托管资产的文本更新同样由 `PersonalKnowledgeFeature` 重新读取并验证页面后执行；Host 只注入带路径租约的机械文件写入端口，成功后发布 `personal_knowledge` 投影变化，不能把知识业务变更冒充为 `mounted_files`。托管资产读取走 feature-owned 窄端口：feature 确认页面身份与可读范围，Host 只做路径解析、目录扫描、MIME 与文本分段，读取结果不携带物理路径。主题创建、归入与取消归类由 actor-aware facade 命令收口：`origin/by` 从真实 actor 推导、主题颜色由 feature 稳定配色策略选择、模型不能提交 `origin/by/locked/actor`；批量归类使用单个 SQLite 事务的全成功/全失败语义并报告 `assigned/unchanged`，Agent 无法取消 `locked` 归类（锁定规则由 feature 强制，用户仍可取消或解锁）。每次成功的托管文本编辑、取消收藏、主题创建与归类变化都会在 SQLite 的 append-only `knowledge_change_records` 中留下含 actor 的轻量业务变更记录，与 `ToolCallResult` 和当前状态分离，禁止从记录回放重建当前状态；`/api/personal-knowledge/change-records` 提供最近变更查询出口。Brain UI 不显示 Agent 徽标或整理提示，默认只展示当前主题与归类。Workbench 数据维护由 Host 提供 `/api/workbench-data/health`、在线一致性备份和恢复暂存；备份同时覆盖 SQLite、知识库托管资产和软件自建空间文件。恢复只在下次启动、任何 feature 打开共享数据库之前应用，并由恢复专用 journal 协调 SQLite、知识资产和软件自建空间文件：commit 前中断会先回滚到原完整集合再重试，commit 后中断只完成收尾；任何无法证明一致的状态都会拒绝打开共享数据库。
...
- 一次性命令使用 `Shell` 并绑定当前 run；后台命令使用 `Shell(background=true)`，默认以 `workspace_session` 生命周期登记到 Host-owned `ProcessRegistry`，返回稳定 `processId`、运行状态、端口事实和 `command-log://` 引用。进程事实显式记录 Conversation、Space、引用和授权模式；取消引用会同步把匹配进程标为 `revoked` 再异步停止，停止不确定或失败保留 `stop_pending`。Conversation/Space 删除在进程确认收口前不会继续。后续由 `ProcessRead` 查询、`ProcessStop` 按稳定身份停止；run release 只清理 `lifetime: "run"`，Panel runtime 关闭时先等待引用撤销清理，再停止进程注册并清理所有仍未结束的 owned 前台/后台进程，因此 `workspace_session` 只跨 run 存活、不跨应用存活。
...
- 历史运行和恢复只读取 `OrdinaryRunState`：run 出生事实、status、Session phase、工具事实、usage 与 timeline 在同一 snapshot 中提交；conversation control 只保存 Session ref、标题、置顶和删除事实。conversation tombstone 一经持久化，该 conversation 的所有 run query、replay 和 stable-terminal 投影立即不可见；evidence、run、managed attachment 与 Pi Session 的物理清理由 Ordinary feature 持有的后台任务执行，阶段失败会在当前进程退避重试，并由后续启动恢复继续。conversation control 缺失或不可投影时，conversation 会整体隔离，queued run 不启动，上述 run 投影同样不可见。WorkView、Panel conversation 和 SSE 都是单向展示投影，不能反向拼装模型历史
- Ordinary 使用 `ordinary-run/v7` snapshot：每个 run 原子写入 `runtime/ordinary-agent/runs/<runId>/snapshot.json`，列表 manifest 只是可重建索引；conversation control 使用独立的 `ordinary-conversation/v3` 文档。`ordinary-run/v5` 只有已显式记录稳定 `memoryOwner`（且存在 Agent Notes 版本时也是同一 owner-scoped 形态）的快照才可迁移：仓储先把原始字节保留为同目录 `snapshot.ordinary-run-v5.json`，再原子迁移为 v7；迁移不增加业务 revision，备份暂不自动删除。真实历史 v5 和 v6 的路径键 Agent Notes 版本都无法可靠映射到稳定 owner，明确视为不兼容，不由当前 cwd、路径、run 或 conversation id 猜测补全。旧 RuntimeDatabase、raw `run.json`、sidecar event/tool-call 和 v4 及更早 schema 仍不读取、不迁移、不双写
...
- 内置工具 canonical name 使用 PascalCase：默认工作区高频工具为 `Read / Write / Edit / Glob / Grep / Shell`；独立的 `create / delete / list` 工具不存在——创建走 `Write`，删除与目录列举走 `Shell` 及其确认门。`Shell` 表示当前配置 shell，不能因参考 Claude Code 而误命名为 `Bash`。专用工具同样使用 PascalCase 前缀分组：`Research* / Web* / Http* / Attachment* / Process* / SkillRead / ReadOutput`，Sub-Agent 工具为 `Agent / AgentSpawn`，MCP 渐进曝光控制工具为 `McpSearch / McpLoad`；MCP 远端工具映射为 `server__tool` 双下划线 canonical 名。不读取旧名称 alias
...
- `Read / Glob / Grep` 使用当前模型 tokenizer 在 producer 端缩小正文或集合页，正常页优先保持在 6,000-token 完整包络内，不借 ToolOutputStore 保存分页状态。HTTP GET 与 `WebFetch` 在 Host store 可用时保留一次响应/页面快照，后续 continuation 从同一 opaque snapshot ref 读取，不重新请求或导航；副作用 HTTP 请求仍禁止透明重放。
- Ordinary run 在线聚合工具定义、执行结果、retained、continuation 和调度指标，只保存 token/字符/字节计数、固定 histogram、状态、原因、耗时和 definition hash，不保存路径、URL、命令、headers 或正文。终态聚合进入可选 `ordinary-run/v7.toolMetrics`；v5 已有指标随 snapshot 迁移，v4 及更早 snapshot 不读取。`/api/runtime/usage-statistics` 与设置统计页从 run snapshot 合并分位数，指标采集失败不得改变工具事实。
- `ResearchRead` 的数组输入是单个工具内部的 producer 扇出，不是普通 Agent 工具轮次或并行工具总量：单批最多 16 个 ref、同时最多读取 4 个；普通 per-ref 失败继续形成 partial facts，取消会停止调度新读取并作为整次工具取消向外传播。Search ref 映射只存在于当前 Research runtime，支持模型在本轮内回读较早结果，并在本轮结束后随 runtime 一起释放。
- `SkillRead` 对 reference、asset 和 script 使用流式文件读取；完整 SHA-256 与 byte/char facts 仍按全文件计算，但内存只保留请求的 reference 字符窗口，取消会终止文件流。
...
- Ordinary 终态 run snapshot 是业务提交点，Session writer 的 revoke/release 在提交后执行；若进程在两者之间退出，启动恢复会在任何 queued successor 获准前，用当前 active branch 上最深的持久化安全 leaf 校准 Session。持久化 run 与 Session 分支无法互相证明时隔离该 conversation，不从未接受的 assistant/partial 分支继续执行
- Ordinary 的模型/工具终态不因 run-scoped resource release 失败而回滚；同一个 idempotent resource lease 会保留到 Session finalization barrier 重试，失败资源释放成功后才释放 Session writer 并开放 queued successor。Lazy MCP 关闭会尝试全部 client，只清除成功断连的引用，失败 client 保留给同一 owner 重试；真实 `McpClientWrapper` 另持有可重试 transport close lease，包括连接构建完成但代次已经失效时尚未挂载到 active client 的 transport。SDK 在初始化失败时发起的未等待 close 只触发同一关闭尝试，正式 release 仍由 wrapper 等待并传播失败，底层句柄只在关闭成功后清除
- Ordinary 启动恢复使用完整 run inventory 契约区分“没有引用”和“未能读取引用”；任一 run 目录缺失、损坏或不兼容都会把 inventory 标为不完整。此时 managed attachment 恢复仍可处理旧 draft 和已知 claim，但必须保留所有现存 conversation 的附件，不能依据部分 run 集合做物理 orphan 删除
...
- 历史 PathMemory / ExperienceCandidate 实验已停用：生产运行不自动采集，不读写 `runtime/path-memory/` 或 `runtime/experience-candidates/`，不暴露对应 HTTP API，也不在设置页挂载。已有目录保持原样，不迁移、不删除；它们只是历史孤儿，不能作为新的“路径依赖”使用或导入。
- Agent 现在拥有模型自主笔记记忆（ADR-0033）：模型可通过 `NoteWrite` 主动全量改写全局笔记或当前 Conversation owner 的笔记，决定什么值得沉淀；owner 只允许稳定的 Space / Workspace 身份，不能由 cwd 或路径哈希推导。新 run 启动时读取 global + 当前 owner 两本 Markdown 笔记，把原文注入系统提示词并随 run birth 保存对应的 SHA-256 内容版本和 owner。`NoteWrite` 的模型输入只接受 `scope: "global" | "owner"`，不能提交任意 Space / Workspace ID；Capability catalog 没有冻结 owner 时，owner 写入明确返回 `memory_scope_unavailable`。`AgentNotesFeature` 对同一 scope 的 Agent 写入串行化，文件仓储在每次 rename 尝试前重新比较当前内容版本；其他 run 或用户直接编辑 `NOTES.md` 后，旧 run 的 `NoteWrite` 会返回当前完整正文与版本，不会自动推进写入基线，模型必须合并后把 `currentVersion` 作为 `baseVersion` 显式重试。缺少冻结版本的旧 run 明确拒绝写入。v6 的路径键笔记不猜测迁移；旧目录保持原样但不再作为当前 owner 记忆读取。Panel runtime-directory lease 排除第二个 AgentArbor 写者；不遵守 lease 的外部编辑器与单次 rename 系统调用之间仍存在普通 Markdown 文件无法消除的极小竞态窗口，不能把该边界表述为跨任意编辑器的绝对线性化 CAS。笔记位于 `runtime/agent-notes/`，用户可直接查看、编辑或删除；工程不自动抄录对话、不自动总结、不替模型决定何时记忆。笔记大小上限为 20,000 字符，超限时工具明确失败并由模型自行精简
- 路径依赖（程序性记忆）已由 `PathDependencyFeature` 落地，owner 与 Agent Notes 一样只使用 `global`、`spaceId` 或 `workspaceId` 的稳定身份。Run birth 只注入当前冻结作用域内的有界记忆目录（最多 16 条 / 2,000 tokens，按当前模型 tokenizer 计数），绝不全量注入方法论正文；模型可按需使用 `MemorySearch`（最多 8 条 / 1,500 tokens）发现、`MemoryRead`（完整结果最多 8,000 tokens）完整读取、`MemoryReference` 明确记录采用，以及 `PathDependencySave` 创建或以 revision 更新。超出完整读取预算时返回真实预算错误，不以摘要冒充正文。模型运行工具只能使用 global + 当前 frozen direct owner。读取和采用事实归 Ordinary，PathDependencyFeature 不建立平行 run 或 read-model。Panel 的独立 Memory Center 可显式选择已登记且未删除的 Space / Workspace，并经 feature facade 校验后管理全局与所选 owner 的路径依赖、展示来源、版本及读取/使用投影；它不接受任意未验证 ownerId。
- 当前没有自动采集任务经验、Dream、向量检索、PathBias 或 Governance/Global Soil 回流；这些能力仍须在真实需求和独立契约稳定后按需出生。
