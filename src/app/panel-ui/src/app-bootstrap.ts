...
export type AppBootstrapState = Pick<AppState, "config" | "tools" | "appUpdate" | "skills" | "conversations" | "deepRuns">;

export async function loadAppBootstrap(): Promise<AppBootstrapState> {
  const [config, tools, mcp, appUpdate, skills, conversations, deepRuns] = await Promise.all([
