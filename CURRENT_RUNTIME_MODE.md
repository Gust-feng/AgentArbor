...
- 历史运行和恢复运行的 read-model 只使用 canonical `run`、runtime `events`、`confirmations`、`contextLedger` 与 run 创建时冻结的 `capabilitySnapshot / informationAccess`；`BasicAgentRun`、WorkView 和 Canvas 都是单向展示投影，不作为恢复事实或持久化缓存
- Ordinary 的 RuntimeDatabase run 持久化使用 `runtime-run-snapshot/v1`：一次保存先校验快照边界，再原子写入 `runtime/runs/<runId>/snapshots/<revision>.json` 完整内容，最后原子切换 `run.json` manifest；读取时再次校验 schema、关键状态、记录归属和 manifest 摘要。`run / workspace / events / modelCalls / toolCalls / artifacts / confirmations / subAgentRuns / contextLedger` 只通过 `saveRunSnapshot` 一次提交，读取不会混合不同 revision 的 sidecar；manifest 只保存窄运行摘要。同一 run 的后台保存与重启后确认恢复共用同一条读改写队列，避免晚到快照覆盖确认决定。每个 run 最多保留 current + previous 两个 revision，conversation 仍独立写入。旧 raw `run.json`、坏 manifest、缺失 snapshot 或 revision/runId 不一致统一返回 `runtime_snapshot_incompatible`（HTTP 410），真实磁盘权限或路径错误保持原始 I/O 失败，不伪装成数据不兼容；不做双读双写或猜测迁移
- Ordinary live 状态只由 run job 的 `status` 决定；正式回答、当前确认和 Context Ledger 作为 Ordinary 自有事实进入终态 payload，Canvas 只消费它们用于展示。正式回答在 run、conversation、WorkView、Canvas 和 RuntimeDatabase 恢复链中保留完整正文，只有额外的 title/summary/preview 展示字段可以有界压缩。`BasicAgentRunStore / BasicAgentRunEventHub` 已删除；`BasicAgentRun` 直接从 job 投影，Panel transport event 只在 `job.streamEvents` 保留一条 live 链
...
- Ordinary 模型上下文中的历史消息、模型压缩摘要和已选 Skill 正文不再先按固定字符数二次裁剪；容量由冻结的模型 token budget 与已有 context compaction 管理。模型请求计量覆盖正文、工具参数、协议扩展和附件元数据，并为无法本地精确计量的二进制附件预留保守预算，provider 仍是最终计量边界；压缩不得拆开 assistant tool call 与对应 tool result。下一轮会从紧邻上一轮 Ordinary run 的同一工具事件 reducer 恢复最多 24 组有界 input/output/error/截断标记，作为模型可见的连续性事实；run/call/event id 仍留在 Context Ledger 与审计引用，不作为工作材料发送给模型。只有紧邻上一轮失败或取消 run 的真实部分输出与稳定停止原因会进入连续性上下文，Panel 追加的错误文案不冒充模型输出。这里不恢复 raw provider 协议对象，不把 `tool-output://` 的 live-only 完整内容伪装成可跨进程读取，也不建设长期记忆；事实已截断、引用已失效或时效性重要时，由模型再次调用原工具
