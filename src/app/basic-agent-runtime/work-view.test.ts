...
test("work view read model keeps ordinary completed answers separate from deliverables", () => {
  ...
  assert.equal(workView.stage, "completed");
...
test("work view read model does not truncate ordinary answers before the chat turn", () => {
  const run = basicRun("completed");
  const longAnswer = `开头\n${"模型正文。".repeat(420)}\n结尾`;
  const workView = createDesktopWorkViewReadModel({
    run,
  ...
  assert.equal(workView.answer?.content, longAnswer);
});

test("work view read model exposes triggered skills without leaking skill body", () => {
  const run = basicRun("completed");
  const workView = createDesktopWorkViewReadModel({
    run,
    events: [event(run.runId, "final.result", "结果已生成", "completed")],
  ...
  assert.deepEqual(workView.triggeredSkills, [
    {
      skillId: "repo-review",
      name: "Repo Review",
  ...
  assert.equal(JSON.stringify(workView.triggeredSkills).includes("FULL PRIVATE SKILL BODY"), false);
  assert.equal(workView.contextLedger.entries.some((entry) => entry.kind === "skill"), true);
});

test("work view read model uses structured skill facts for injected and omitted states", () => {
  const run = basicRun("completed");
  const workView = createDesktopWorkViewReadModel({
    run,
    events: [event(run.runId, "final.result", "结果已生成", "completed")],
  ...
  assert.deepEqual(workView.triggeredSkills.map((skill) => ({
  ...
  assert.equal(workView.triggeredSkills[0]?.selection?.selectionMethod, "model");
  assert.equal(workView.triggeredSkills[0]?.selection?.modelCallRef, "model-call:skill-router-2");
  assert.deepEqual(workView.triggeredSkills[0]?.selection?.candidateSkillIds, ["repo-review", "bulky-review", "writer"]);
  assert.deepEqual(workView.triggeredSkills[0]?.selection?.selectedSkillIds, ["repo-review"]);
  assert.equal(workView.triggeredSkills[0]?.selection?.omittedReasons?.[0]?.code, "selection_limit");
  assert.equal(workView.triggeredSkills[0]?.selection?.rejectedReasons?.[0]?.skillId, "writer");
  assert.equal(workView.triggeredSkills[0]?.selection?.confidence, 0.88);
  assert.equal(workView.triggeredSkills[0]?.selection?.reasonSummary, "模型选择 repo-review 作为本轮唯一注入技能。");
  assert.equal(JSON.stringify(workView.triggeredSkills).includes("FULL PRIVATE SKILL BODY"), false);
});

test("work view read model surfaces failed skill loading without body content", () => {
  const run = basicRun("completed");
  const workView = createDesktopWorkViewReadModel({
    run,
    events: [],
  ...
  assert.equal(workView.triggeredSkills[0]?.loadStatus, "failed");
  assert.equal(workView.triggeredSkills[0]?.injectionStatus, "failed");
  assert.equal(workView.triggeredSkills[0]?.error, "技能正文文件不存在。");
  assert.equal(JSON.stringify(workView.triggeredSkills).includes("FULL PRIVATE SKILL BODY"), false);
});

test("work view read model surfaces approval as the main stage", () => {
  ...
  assert.equal(workView.stage, "awaiting_approval");
  assert.equal(workView.pendingConfirmation?.confirmationId, "confirmation-test");
  assert.equal(workView.pendingConfirmation?.actionSummary, "准备删除文件。");
  assert.equal(workView.pendingConfirmation?.actionSummary.includes("批准后只执行本次删除"), false);
  assert.equal(workView.deliverable, undefined);
});

test("work view read model does not keep stale canvas confirmation after approval resumes", () => {
  ...
  assert.notEqual(workView.stage, "awaiting_approval");
  assert.equal(workView.pendingConfirmation, undefined);
  assert.equal(workView.workSummary.pendingActionCount, 0);
  assert.equal(workView.transcriptNodes?.some((node) => node.kind === "confirmation"), false);
});

test("work view read model keeps unfinished status matrix explicit without stale confirmations", () => {
  const cases: readonly {
    ...
    readonly expectedStage: ReturnType<typeof createDesktopWorkViewReadModel>["stage"];
  ...
  for (const item of cases) {
    ...
    assert.equal(workView.stage, item.expectedStage, item.name);
    assert.equal(workView.pendingConfirmation !== undefined, item.expectsPendingConfirmation, item.name);
    assert.equal(workView.workSummary.pendingActionCount, item.expectsPendingConfirmation ? 1 : 0, item.name);
    if (!item.expectsPendingConfirmation) {
      assert.equal(workView.transcriptNodes.some((node) => node.kind === "confirmation"), false, item.name);
      assert.notEqual(workView.stage, "awaiting_approval", item.name);
      assert.notEqual(workView.stage, "completed", item.name);
      assert.equal(JSON.stringify(workView).includes("正在处理"), false, item.name);
    }
  }
});

test("work view read model preserves concrete confirmation action", () => {
  const run = basicRun("approval_needed");
  const workView = createDesktopWorkViewReadModel({
    run,
    events: [
  ...
  assert.equal(workView.pendingConfirmation?.actionSummary, "删除文件：C:\\repo\\old.txt");
  assert.equal(workView.transcriptNodes?.some((node) => node.confirmation?.actionSummary === "删除文件：C:\\repo\\old.txt"), true);
});

test("work view transcript only emits a confirmation node for the current pending confirmation", () => {
  const run = basicRun("running");
...
test("work view visible events preserve product activity instead of tail model deltas", () => {
  ...
  assert.equal(workView.visibleEvents.some((item) => item.type === "tool.completed"), true);
  assert.equal(workView.visibleEvents.some((item) => item.type === "run.started"), false);
  assert.equal(workView.visibleEvents.some((item) => item.type === "model.output.delta"), false);
  assert.equal(workView.visibleEvents.some((item) => item.type === "model.output.completed"), false);
  assert.equal(workView.visibleEvents.some((item) => item.type === "final.result"), false);
  assert.equal(workView.currentAction, "file notes.md · 12 bytes");
});

test("work view read model closes merged reasoning on completion event", () => {
  const run = basicRun("completed");
  const events: RunEvent[] = [
    {
  ...
  const workView = createDesktopWorkViewReadModel({ run, events });
  const thinking = workView.transcriptNodes?.find((node) => node.kind === "thinking");
...
test("work view transcript preserves reasoning whitespace without a visible label", () => {
  const run = basicRun("completed");
  const events: RunEvent[] = [
    {
...
test("work view transcript keeps early tool activity beyond visible event window", () => {
  const run = basicRun("completed");
  ...
  assert.equal(workView.visibleEvents.some((item) => item.id === earlyToolResult.id), false);
  assert.equal(workView.transcriptNodes?.some((node) => node.eventType === "tool.completed" && node.toolName === "read_file"), true);
});

test("work view transcript projection owns reasoning node merging", () => {
  const run = basicRun("completed");
  const events: RunEvent[] = [
    {
      ...event(run.runId, "model.reasoning.delta", "first", "running"),
...
test("work view transcript preserves model side output before tool calls", () => {
  const run = basicRun("completed");
  const events: RunEvent[] = [
    {
...
test("work view transcript suppresses ordinary startup and placeholder events", () => {
  const run = basicRun("running");
...
test("work view transcript carries tool names for readable workflow actions", () => {
  const run = basicRun("completed");
  const nodes = transcriptNodesFromRunEvents([
    {
...
test("work view read model completes live reasoning after interleaved output", () => {
  const run = basicRun("completed");
  const events: RunEvent[] = [
    {
      ...event(run.runId, "model.reasoning.delta", "first", "running"),
  ...
  const workView = createDesktopWorkViewReadModel({ run, events });
  const thinking = workView.transcriptNodes?.filter((node) => node.kind === "thinking" && node.eventType?.startsWith("model.reasoning"));
...
test("work view read model settles reasoning when the model turn ends without explicit completion", () => {
  const run = basicRun("completed");
  const events: RunEvent[] = [
    {
      ...event(run.runId, "model.reasoning.delta", "first", "running"),
...
test("work view read model does not promote restored summaries into chat deliverables", () => {
  const run = basicRun("completed");
  const workView = createDesktopWorkViewReadModel({
    run,
...
test("work view context ledger distinguishes blocked context refs", () => {
  const run = basicRun("running");
  const workView = createDesktopWorkViewReadModel({
    run,
    events: [],
  ...
  assert.equal(workView.contextAttachments[0]?.status, "blocked");
  assert.equal(workView.contextLedger.entries.some((entry) => entry.status === "blocked"), true);
});

test("work view context ledger keeps budget details out of ordinary summaries", () => {
  const run = basicRun("running");
  const workView = createDesktopWorkViewReadModel({
    run,
    events: [],
  ...
  const budgetEntry = workView.contextLedger.entries.find((entry) => entry.kind === "budget");
  const omittedEntry = workView.contextLedger.entries.find((entry) => entry.status === "omitted");
  const text = JSON.stringify(workView.contextLedger);
...
test("work view current action does not mirror the user task text", () => {
  ...
  assert.equal(workView.headline, "");
  assert.equal(workView.currentAction, "");
  assert.equal(workView.currentAction.includes("sk-user-task-secret"), false);
  assert.equal(workView.currentAction.includes("目标"), false);
});

test("work view current action skips generic approval resume events", () => {
  const run: BasicAgentRun = {
    ...basicRun("running"),
    currentStep: "继续处理。",
  };
  ...
  assert.equal(workView.visibleEvents.some((item) => item.type === "user_approval.received"), false);
  assert.equal(workView.visibleEvents.some((item) => item.type === "run.resumed"), false);
  assert.equal(workView.currentAction, "pnpm test · 通过");
});

test("work view transcript omits approved resume nodes but keeps denied decisions", () => {
  const run = basicRun("running");
  const nodes = transcriptNodesFromRunEvents([
    {
      ...event(run.runId, "user_approval.received", "已继续。", "running"),
