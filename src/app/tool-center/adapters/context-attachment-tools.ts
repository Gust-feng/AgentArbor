...
export function createReadContextAttachmentImageTool(options: ContextAttachmentToolOptions = {}): ToolExecutor {
  return {
    definition: {
...
async function fileHasNulByte(filePath: string, size: number): Promise<boolean> {
...
function imageModelFilename(entry: AttachmentEntry, target: AttachmentTarget): string {
  ...
  const title = attachmentTitle(entry);
  return path.basename(title) || "attachment-image";
}

function imageMimeTypeForTarget(ref: TaskSoilContextRef, target: AttachmentTarget): string | undefined {
  const extension = tableTargetExtension(ref, target.targetPath);
  const mimeFromExtension = IMAGE_MIME_BY_EXTENSION[extension];
  ...
  if (metadataMimeType !== undefined && Object.values(IMAGE_MIME_BY_EXTENSION).includes(metadataMimeType)) {
...
async function searchPath(input: {
  readonly absolutePath: string;
  readonly rootAbsolutePath: string;
...
function createSearchFacts(): SearchFacts {
  return {
    searchedFiles: 0,
    skippedFiles: 0,
