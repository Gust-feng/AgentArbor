...
会话与运行持久化由 `OrdinaryAgentFeature` 自己负责。它只从 `ordinary-conversation/v3` 的 Session ref、冻结 Conversation owner 和 Pi active leaf 读取上下文，并在 `ordinary-run/v7` snapshot 中保存 Session refs、冻结 owner/Agent Notes 版本、工具事实、usage、状态和展示 checkpoint；Panel、旧 RuntimeDatabase 和 UI read-model 都不是恢复来源。v5 只有已经显式记录稳定 `memoryOwner`，且其 Agent Notes 版本（若有）与同一 owner 对齐时才迁移；真实历史 v5 和 v6 的路径键版本都没有稳定 owner 映射，直接视为不兼容，不能从 cwd、路径或会话身份猜测补全。
...
能够完整内联的工具事实直接保存在 `ordinary-run/v7`。超过内联边界的完整正文、结构化 JSON 或失败证据写入 Host-owned `runtime/tool-evidence/`，run snapshot 只保存预览、稳定 `tool-output://` 引用、字符数、byte length、SHA-256 与 continuation 输入。该引用跨进程可读；完整读取不会删除审计证据，删除所属 conversation 时才由 Ordinary 按 run owner 回收。
...
- 附件原始字节不写入 Ordinary run snapshot、事件或 Panel read-model；支持的 inline image 由 Pi durable Session 持有并在后续轮次重建。无法由当前 Pi 公共消息契约无损表达的附件必须形成明确 delivery failure，不能静默删除后继续报告成功。用户输入或会话历史中的图片在模型不支持图像输入时，以可观察文本占位替换后继续运行（占位只声明图片未交付与原因，不伪造、不摘要图片内容）；即使首轮就是 text-only 模型，图片字节仍解析进 Pi durable Session，切换视觉模型后可以重建读取；工具产物图片仍必须形成可观察失败，不适用占位替换。未知 provider 私有字段、悬空 continuation 和无法证明结果的内部执行对象一律不持久化，不能为了“续跑”伪造 tool call/result 对。
