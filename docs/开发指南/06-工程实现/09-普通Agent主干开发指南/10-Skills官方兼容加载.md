...
9. `evals/` 本地质量 artifact：loader/doctor 可以索引 `evals/*`、校验、统计并提示缺失，但 `evals/` 不属于运行时资源，不进入 frozen runtime resource index，不进入 Context Ledger / Context Pack，也不能通过 `read_skill_resource` 读取。
