...
当前生产链固定为 `request-handler -> ordinary-routes -> OrdinaryAgentFeature -> Pi AgentHarness/Session adapter -> ToolCenter`。`OrdinaryAgentFeature` 独占 command/query/event、`ordinary-run/v4`、Session refs 和单向 read-model；旧 BasicAgent、Desktop session、Panel run job 与 `MinimalRuntime` 不再是实现入口。
