...
本目录是 AgentArbor 开发前的正式指南。它采用分册结构，当前产品方向是桌面通用 Agent：用户通过统一 Desktop Shell 提交任务和工作区上下文，系统形成 Task Soil，经地下认知运行时成形 Plan，再由地上执行运行时交付 Fruits，并把可复用经验经过治理回流 Global Soil。

开发指南不是过程归档、版本路线图或会议纪要。它只写稳定结论、工程边界和可执行契约。

当前产品架构事实源是 [ADR-0022-AgentArbor桌面通用Agent与双运行时架构](../架构设计/产品架构/ADR-0022-AgentArbor桌面通用Agent与双运行时架构.md)。[ADR-0018-AgentArbor原生概念树架构](../架构设计/产品架构/ADR-0018-AgentArbor原生概念树架构.md) 保留历史脉络和术语背景；被产品架构索引标记为当前决策的 ADR 可作为 ADR-0022 的下位细化，研究资料只作为背景输入。
...
- 用户只面对 Desktop Shell 一个产品入口，不直接面对 Underground 或 Aboveground 两个入口。
...
- Aboveground Execution Runtime 负责执行智能：消费已成形 Plan，进行文件修改、文档生成、原型制作、工具调用和验证。
