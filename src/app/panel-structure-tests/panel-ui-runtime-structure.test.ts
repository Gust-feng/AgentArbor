...
test("panel React workbench consumes Ordinary Agent projection APIs", async () => {
  ...
  assert.equal(appRunController.includes("loadSettledRunProjection"), true);
  assert.equal(appRunController.includes("loadObservedRunReadModel"), false);
  assert.equal(appRunController.includes("appStateWithSettledRunProjection"), true);
  assert.equal(appRunController.includes("workView: options.app.workView"), true);
  assert.equal(appRunController.includes("capabilityResolution: options.app.capabilityResolution"), true);
  assert.equal(appRunController.includes('from "./run-capability-state"'), false);
  assert.equal(appRunController.includes("nextRunCapabilityState(previous, {"), false);
  assert.equal(appRunController.includes("run: response.run"), true);
  ...
  assert.equal(transcriptTimeline.includes("agent-record-selectable"), true);
  assert.equal(transcriptTimeline.includes("agent-confirmation-activity"), true);
  assert.equal(transcriptTimeline.includes("agent-record-icon"), true);
