...
- `src/`：AgentArbor TypeScript 实现代码。当前优先打磨本地 Desktop Shell / Panel、默认普通桌面 Agent 运行链、模型/工具/确认/事件/持久化基础设施；deep / Agent 集群作为长期能力边界保留，不新增默认可见入口，也不主动改动后端路径。
...
- deep / Agent 集群是长期能力边界：当前不做默认可见 deep 入口，不主动改动 deep 后端；后续若重启，应继续复用 AgentTurnRuntime、ToolCenter、RunEvent、RuntimeDatabase、Confirmation Gate 等基础设施，不能另起平行运行时。
