...
当前正式 Ordinary 闭环由 `OrdinaryAgentFeature` 通过 `ordinary-run/v4` snapshot 保存业务运行事实，并由 Pi Session active branch 保存跨轮模型事实；旧 RuntimeDatabase 不是 Ordinary 的恢复来源。Multi-Agent 继续拥有独立 Deep store 和 run-tree 契约，两类状态不互转。

本文件中的 `TaskSoil`、`Plan`、`PlanPackage`、`ExecutionPlan`、`NutrientRequest`、`NutrientPatch`、`PlanRevision`、`RunMemory`、`ExperienceCandidate` 和 `PathBias` 是长期能力契约，不是每次 Ordinary 请求的必备结构。只有真实功能和稳定消费者出生后才接入，不得为了保持早期骨架而恢复全局流水线。
