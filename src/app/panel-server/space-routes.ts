...
export async function handlePanelSpaceRoute(
  ...
  if (treeMatch !== null && request.method === "DELETE") {
    ...
    await feature.commands.deleteSpace(spaceId);
  ...
  if (previewReference !== null && request.method === "GET") {
    ...
    const preview = item.reference.kind === "workbench_asset"
      ? await getWorkbenchAssetPreview({ get: runtime.workbenchAssetFeature.queries.get }, item.reference.assetId, item.id)
  ...
  if (referenceContent !== null && request.method === "PUT") {
    ...
    if (item.reference.kind === "workbench_asset") {
      writeJson(response, 200, { ok: true, preview: await updateWorkbenchAssetTextPreview(
        { updateText: runtime.workbenchAssetFeature.commands.updateText },
...
/** 受管内容（第二阶段 Content Vault 同步）错误到 HTTP 的错误映射。 */
export function managedContentHttpError(error: import("../managed-content/index.js").ManagedContentError): PanelHttpError {
  switch (error.code) {
    case "managed_content_root_not_found":
    case "managed_content_space_not_found":
    case "managed_content_entry_not_found":
      return new PanelHttpError(404, error.code, error.message);
    case "managed_content_invalid_path":
    case "managed_content_path_escape":
    case "managed_content_entry_exists":
    case "managed_content_not_text":
    case "managed_content_revision_conflict":
      return new PanelHttpError(409, error.code, error.message);
    case "managed_content_released":
      return new PanelHttpError(503, "panel_runtime_quiescing", "面板正在关闭，不能接受新的请求。");
    case "managed_content_io_failure":
      return new PanelHttpError(500, error.code, error.message);
  }
  return new PanelHttpError(500, "managed_content_io_failure", error.message);
}
