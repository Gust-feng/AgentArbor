...
test('varies ambient copy across the week and broad day periods', () => {
  const copies = [
  ...
  expect(new Set(copies).size).toBeGreaterThanOrEqual(6)
})

test('keeps generated copy declarative and substantial', () => {
  ...
  expect(copies.every((copy) => copy.length >= 18)).toBe(true)
  expect(copies.every((copy) => !/[?!？！]/u.test(copy))).toBe(true)
})
