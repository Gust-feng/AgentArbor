...
test("panel UI contracts stay split by product concern", async () => {
  ...
  assert.equal(contractRun.includes('from "../../../panel-read-model/ordinary-agent-run-contracts"'), true);
  assert.equal(contractRun.includes("OrdinaryPanelReplay"), true);
  assert.equal(contractRun.includes("OrdinaryPanelRunDetail"), true);
  assert.equal(contractRun.includes("OrdinaryPanelRunView"), true);
  assert.equal(contractRun.includes("export type DesktopRunDetail = OrdinaryPanelRunDetail"), true);
  assert.equal(contractRun.includes("export type BasicAgentReplay = OrdinaryPanelReplay"), true);
  assert.equal(contractRun.includes("export type BasicAgentRunView = OrdinaryPanelRunView"), true);
  assert.equal(contractRun.includes("basic-agent-run-view-contracts"), false);
  assert.equal(contractRun.includes("PanelRunResultReadModel"), false);
  assert.equal(contractRun.includes("readonly result?:"), false);
  assert.equal(contractRun.includes('Omit<OrdinaryPanelRunView, "workSession">'), false);
  ...
  assert.equal(contractConfig.includes("readonly modelCapabilityProfiles?: readonly ModelCapabilityProfile[]"), true);
  assert.equal(contractConfig.includes("export type ModelCapabilityProfile"), true);
  ...
  assert.equal(appConfigProjection.includes("modelCapabilityProfiles: incoming.modelCapabilityProfiles ?? previous?.modelCapabilityProfiles"), true);
  assert.equal(contractContext.includes("export type ContextAttachment"), true);
  assert.equal(contractContext.includes("readonly mediaPreview?:"), true);
  assert.equal(contractContext.includes('readonly kind: "image";'), true);
  assert.equal(contractConversation.includes("export type Conversation"), true);
  assert.equal(contractConversation.includes("export type ConversationTurnAttachment"), true);
  assert.equal(contractConversation.includes("readonly attachments?: readonly ConversationTurnAttachment[]"), true);
  ...
  assert.equal(contractSkills.includes("readonly sourcePath?: string"), false);
  assert.equal(contractSkills.includes("readonly sourceKind?:"), true);
  assert.equal(contractSkills.includes("readonly sourceRootId?: string"), true);
  assert.equal(contractSkills.includes("readonly stateKey?: string"), true);
  ...
  assert.equal(types.includes("McpServerCatalogItem"), true);
