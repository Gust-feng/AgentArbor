...
export type AppBootstrapState = Pick<AppState, "config" | "tools" | "appUpdate" | "skills" | "subAgents" | "conversations" | "deepRuns">;

export async function loadAppBootstrap(): Promise<AppBootstrapState> {
  const [config, tools, mcp, appUpdate, skills, subAgents, conversations, deepRuns] = await Promise.all([
