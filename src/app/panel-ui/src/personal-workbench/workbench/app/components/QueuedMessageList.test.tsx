...
test('shows queued messages and supports edit and recall', () => {
  ...
  const onGuide = vi.fn().mockResolvedValue(true)

  render(<QueuedMessageList messages={messages} onRemove={onRemove} onUpdate={onUpdate} onGuide={onGuide} />)
  ...
  fireEvent.click(screen.getByRole('button', { name: '引导待发送消息：先整理这份报告' }))
  expect(onGuide).toHaveBeenCalledWith('queued-1')
})
