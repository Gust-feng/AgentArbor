...
第一阶段以内存运行时和最小事件链证明 Agent 工作闭环，并通过 RuntimeDatabase 保存 Run、Event、ToolCall、ModelCall 和 Artifact 的运行投影。A2A Adapter、完整 MCP Adapter、AG-UI Adapter、长期记忆治理、自主演化、复杂权限系统、完整 ORM/迁移体系和生产级数据库后端按后续 Full Profile 演进。
