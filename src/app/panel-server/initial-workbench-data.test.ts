...
test("Panel 全新首次启动把内置内容创建为普通 Space 托管文件和知识收藏", async () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(learningSpace?.entries.length, 3);
      assert.deepEqual(
        learningSpace?.entries.map((entry) => entry.item.title),
        ["2026年学习资料", "阅读笔记", "CS231n 课程主页"],
      );
      assert.deepEqual(
        learningSpace?.entries.map((entry) => entry.item.reference.kind).sort(),
        ["managed_folder", "managed_folder", "web_page"],
      );
      ...
      assert.deepEqual([...diagram.subarray(0, 8)], [137, 80, 78, 71, 13, 10, 26, 10]);
      assert.match(await fs.readFile(path.join(notes.reference.path, "Transformer 精读.md"), "utf8"), /自注意力/u);
      assert.match(await fs.readFile(path.join(notes.reference.path, "train_loop.py"), "utf8"), /optimizer\.step/u);
      ...
      assert.equal(knowledge.assignments.length, 8);
...
test("首次内置内容导入失败后沿同一安装批次继续且不复制文件或知识页面", async () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal((await runtime.spaceFeature.queries.getTree("space-learning"))?.entries.length, 3);
