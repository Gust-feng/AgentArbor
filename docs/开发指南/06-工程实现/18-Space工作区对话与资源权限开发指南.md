...
Space 引用是常驻授权上下文，不是本轮附件：注入的图片引用只作为模型可读的文件引用进入 Task Soil，**不自动解析为模型视觉附件**，因此每个新 turn 不会悄悄携带旧图片；视觉模型需要看图时按需调用 `AttachmentReadImage`（text-only 模型下该工具返回明确失败）。只有用户本轮明确选择或上传的图片（`ctx:` / `uploaded-attachment:` 附件）才自动成为本轮模型视觉输入。
