...
- continuation 只读取工具输出顶层 `continuation / continuations`。小结果完整返回且不附带从头重读入口；大结果由 producer 返回首段、截断事实和从未读位置开始的可执行续读输入。任何真实分页/offset ceiling 或副作用请求无法安全重放时，都必须返回明确失败事实，不得返回 `completed + truncated` 的不可续读结果，也不得生成会重放 POST/PUT/DELETE 的 synthetic continuation。超过共享 transport budget 且没有 continuation 时明确失败
- MCP adapter 只保留服务端原生 `content[]` 与可选 `structuredContent`，不生成 `summary / mcpResult / result` 多份包装；只对“text 可解析 JSON 且与 structuredContent 深度完全相等”的精确镜像做无损去重，其他内容完整保留；`isError=true` 成为正式工具失败，图片 base64 不进入 JSON
