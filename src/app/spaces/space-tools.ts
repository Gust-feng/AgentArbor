...
export function createSpaceMoveTool(options: SpaceToolOptions): ToolExecutor {
  return tool({
    name: "SpaceMove",
    description: "Move a Space folder or reference item within or between Spaces. Moving changes only SpaceTree metadata; it never moves a local file or changes a conversation owner.",
    metadata: writeMetadata,
    inputSchema: requiredSchema({
      targetKind: { type: "string", enum: ["folder", "reference"] }, targetId: { type: "string" }, destinationSpaceId: { type: "string" }, destinationFolderId: { type: "string" },
    ...
    execute: async (input) => {
      ...
      const destinationFolderId = optionalString(record.destinationFolderId);
      if (target === undefined || destinationSpaceId === undefined || destinationFolderId === null) return invalid("targetKind, targetId and destinationSpaceId are required strings; destinationFolderId must be omitted or a string.");
      return resultFor(() => options.spaces.commands.move({ target, destinationSpaceId, destinationFolderId }), () => ({ status: "moved", target, destinationSpaceId, destinationFolderId }));
    },
  });
}
...
export function createSpaceAddReferenceTool(options: SpaceToolOptions): ToolExecutor {
  return tool({
    ...
    metadata: writeMetadata,
    inputSchema: requiredSchema({
      spaceId: { type: "string" }, parentFolderId: { type: "string" }, title: { type: "string" }, reference: referenceSchema,
    ...
    execute: async (input) => {
      ...
      if (spaceId === undefined || title === undefined || parentFolderId === null || reference === undefined) return invalid("spaceId, title and a valid reference are required; parentFolderId must be omitted or a string.");
      return resultFor(() => options.spaces.commands.addReference({ spaceId, parentFolderId, title, reference }), (item) => ({ status: "added", item }));
    },
  });
}
...
export function createSpaceRenameTool(options: SpaceToolOptions): ToolExecutor {
  return tool({
    name: "SpaceRename",
    description: "Rename a Space, Space folder, or Space reference display label. It does not rename any external file, artifact, or conversation.",
    metadata: writeMetadata,
    inputSchema: requiredSchema({ targetKind: { type: "string", enum: ["space", "folder", "reference"] }, targetId: { type: "string" }, title: { type: "string" } }, ["targetKind", "targetId", "title"]),
    execute: async (input) => {
      const record = asRecord(input);
...
function targetFrom(kind: unknown, id: unknown): SpaceTarget | undefined {
  ...
  if (targetId === undefined || (targetKind !== "space" && targetKind !== "folder" && targetKind !== "reference")) return undefined;
  return { kind: targetKind, id: targetId };
}

function movableTarget(kind: unknown, id: unknown) {
  const target = targetFrom(kind, id);
  return target?.kind === "folder" || target?.kind === "reference" ? target : undefined;
}
...
function isExpectedSpaceOperationError(code: SpaceFeatureError["code"]): boolean {
  return code === "space_not_found" || code === "space_folder_not_found" || code === "space_reference_not_found" || code === "space_parent_not_found" || code === "space_invalid_move" || code === "space_invalid_input" || code === "space_id_collision";
}
