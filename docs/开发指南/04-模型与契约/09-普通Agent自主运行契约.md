...
工具结果回到模型时，应优先提供足以继续开发判断的真实内容。UI 可以额外展示摘要字段，但摘要不能替代模型继续工作所需的正文、stdout/stderr、文件片段或错误信息。
...
普通 Agent 的输出预算应来自模型能力目录或用户显式配置。工程层可以在未知模型能力时使用保守默认值，但不能给所有普通会话套一个固定小输出上限；长回答、报告、代码解释和多步结果应由模型能力、上下文窗口、用户要求和运行边界共同决定。
