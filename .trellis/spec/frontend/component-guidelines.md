...
- 首屏必须是 IDE / workbench 式运行入口：左侧浅灰导航、中央白色方向画布、右侧 inspector、底部输入框和可展开设置 / 调试详情。默认空态主标题为 `把想法长成方向`，副标题为 `地下组织先理解、探索、收束，再交给地上生长`，能力入口为 `网页研究`、`代码理解`、`证据整理`、`方向交接`。
...
- 工作流阶段、rootlet kind、模型调用追踪、模型输出、Agent Transcript、EventLog 与 Observation Snapshot 都属于详情 / inspector / debug 信息；完成态中央主画布可以展示方向判断、收束解释和方向交接摘要，但不得把 raw EventLog、Observation JSON、model/tool refs 作为主内容大块压在首屏。
...
- 点击启动后即使没有 SSE / WebSocket，也必须先渲染运行中骨架，然后调用 async run API 创建 job，并每 1-2 秒 polling 当前 run；轮询响应应驱动事件游标、等待点、Agent Transcript 和模型调用状态，避免真实 provider 等待期间页面空白。
- Agent Transcript 只能展示可审计工作笔记，例如观察、动作、产出、依据、下一步和引用；模型输出区可以展示通过 `outputContract` validation 和 `visibleOutput.fieldTypes` 展示策略的 visible output 安全投影，例如 option 的 summary / tradeoffs / applicability、risk 的 impactScope / severity / mitigation 和其他 rootlet kind 的契约字段。它不得展示隐藏思维链、完整 prompt、provider raw response、API key、token、未清洗错误、未校验模型输出或 rootlet parser 会拒绝的候选字段。
