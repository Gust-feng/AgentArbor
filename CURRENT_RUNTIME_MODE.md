...
`SpaceFeature` 是独立的内容组织功能：它持有 Space 元数据、软件资产和外部 Workspace 引用。`workspace_folder` 是用户文件系统中的只读权限引用；同一 Workspace 可被多个 Space 独立引用，同一 Space 内禁止重复或父子嵌套引用。Workspace 没有跨 Space 移动操作，目标 Space 需要使用时重新添加；移除引用只撤销当前 Space 权限，不删除外部源内容。`managed_folder`、Workbench 文档、生成产物等是 AgentArbor 自己维护的软件资产，用户和模型可通过结构化工具编辑、重命名和移动；删除 Space 或软件资产时才物理删除这些资产并经过删除确认。Conversation 是 Space 的固定 owner，不能作为普通引用添加、移动或解绑，只能删除。目标口径（ADR-0035）中 Conversation owner 为 Space 或 Workspace 的判别联合，当前实现仍以 Space 引用表达 owner。外部路径型引用在添加时保存平台文件身份（当前实现为 `dev + inode`），并通过引用事件与权限事实协调；软件自有资产不使用这项外部解绑规则。Space 列表和树查询不探测磁盘，真实的打开、预览、正文操作、附件工具、视觉附件交付和标准文件工具访问才校验引用根。确认目录或文件根消失、类型变化或来源身份不符后，系统删除当前 Space 的外部引用、撤销活动 Run 并收口关联进程；不显示可恢复的 `unavailable` 节点，原路径恢复也不会恢复旧身份，重新添加才生成新 `referenceId`。目录下的文件和子目录由真实文件系统扫描、读取和修改，不作为逐项数据库记录。Host 在每轮提交时从 Conversation owner 解析所属 Space，把当前软件资产和 Workspace 引用（包括后端来源身份）冻结为本轮资源与精确路径授权；后续新增引用只影响后续轮次，撤销引用通过 live deny overlay 立即阻止活动 Run 的后续访问。模型继续使用标准 `Read / Glob / Grep / Write / Edit` 和真实绝对路径；Host 向这些成熟工具注入 Run-scoped path authorization，后端保留 `referenceId` 作为权限、进程关联和审计事实，不再注册 `SpaceWrite / SpaceEdit` 第二套文件 API。普通模式的文件工具只能命中本轮冻结且当前未撤销的引用，`full_access` 可访问引用集合外的真实路径，但仍不能访问已撤销引用。结构化 Space 工具的创建、添加、重命名、移动和引用移除默认执行；删除 Space、Conversation 或软件资产进入确认门。Panel 只消费 query projection。

Workbench 的结构化业务数据保存在 `runtime/workbench.sqlite3`。`SpaceFeature` 独立拥有 `spaces / space_references` 表；Host 另有 `space_conversation_links` 和 `space_conversation_deletions` 表，只保存跨 Space/Ordinary 工作流的身份和检查点，不复制 feature 正文或状态。前者协调新 Conversation 的 owner link 与单独删除：创建按 `prepared -> owner_linked -> conversation_created`，删除按 `prepared -> processes_stopped -> conversation_deleted -> reference_unlinked`；恢复只收尾已有事实，链接身份变化时 fail closed。后者协调 Space 级联删除，按 `prepared -> processes_stopped -> conversations_deleted -> space_deleted` 推进，失败保存 `resumeFrom`。Panel 在监听端口前恢复所有未完成记录；删除中的 Space 或 Conversation 拒绝新的提交。进程无法确认停止时记录 `stop_pending` 并阻止继续删除；外部 Workspace 始终保留。
