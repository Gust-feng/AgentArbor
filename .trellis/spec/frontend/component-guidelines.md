...
- 首屏必须是 Desktop Shell 任务入口：左侧任务记录，中央 Main Canvas + transcript，底部任务输入框，右侧运行状态、Agent Run Tree inspector 和可折叠设置 / 调试详情。默认空态主标题为 `Desktop Shell 工作台`，主画布展示 Plan / Fruit 空态；不得再把 `网页研究`、`代码理解`、`证据整理`、`方向交接` 作为固定能力卡片模板。
