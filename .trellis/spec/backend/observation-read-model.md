...
- Desktop canvas 可以展示 Task Soil 的 goal summary、context refs、permission boundary refs 和只读短 preview；这些字段必须来自 Task Soil 安全投影，preview 必须截断、脱敏，并且不得包含 raw file body、runtime/store refs、API key、token、Authorization、raw prompt、provider raw response 或 raw tool output。
...
| Desktop canvas 的 Task Soil context preview 泄漏 raw file body、API key、token、Authorization 或 runtime/store ref | 测试失败；必须改为 refs、短摘要和脱敏 preview |
...
| Desktop Shell 真实 AI 输出契约失败只返回 `panel_internal_error`，缺少 purpose / contract id / failure kind / call ref 诊断 | 测试失败；必须从 EventLog model.requested/model.failed 安全字段派生可操作失败摘要 |
...
- 本地 panel response 覆盖 AI 禁用模式拒绝、fake AI、openai-compatible 配置失败、真实 AI 输出契约失败安全诊断、sync run 兼容、async run job、partial / final event cursor、SSE stream、cursor 续传、stream 断开后后台 run 完成、tracking read model、transcript 和 model visible output，并证明 HTTP JSON / SSE 不包含 raw secret、token、完整模型 prompt、provider raw response、hidden reasoning、raw tool output、app parser 会丢弃的候选字段或未校验模型输出。
- 本地 panel response 覆盖 Desktop canvas 的 Task Soil context refs、permission refs、只读 preview、preview 截断与脱敏，以及 unauthorized context/permission refs 的 400 边界。
