...
Space/Workspace 资源权限的事实源是 [ADR-0034](docs/架构设计/产品架构/ADR-0034-Space工作区引用与对话资源生命周期.md) 和 [工程指南](docs/开发指南/06-工程实现/18-Space工作区对话与资源权限开发指南.md)：Space 只引用外部 Workspace，Conversation 固定属于 Space，模型看到真实路径，后端保留 `referenceId`，普通 Shell 每次确认，`full_access` 同时覆盖 Shell 和文件路径。本文其余段落中的 `workspaceRoot` 与 Task Soil 表述都按这一口径理解，不存在第二套并行口径。
...
- Panel 首页负责选择 Space；“从 Workspace 开始”只是创建一个只引用该 Workspace 的新 Space 的快捷入口。Conversation 创建后固定属于该 Space，不能在对话中切换 Space 或 Workspace；Run 冻结当时资源快照，模型上下文展示真实绝对路径。
...
- 普通 `agent` 的 Space 文件权限来自 Conversation 唯一 Space owner 在 Run 创建时生成的资源快照；不同 Space 的引用不会合并，新增引用只影响新 Run，`status` 非 `available` 的引用不进入冻结授权。模型看到真实绝对路径，后端仍保存 `referenceId` 作为权限与审计事实。当前撤销引用只对后续 Run 生效，尚未实现阻断进行中 Run 的即时 deny overlay，该项差异见 [工程指南](docs/开发指南/06-工程实现/18-Space工作区对话与资源权限开发指南.md)。
