...
`ADR-0020-智能通道与模型接入边界.md`、`docs/开发指南/06-工程实现/03-人工智能与确定性边界.md` 和对应 Trellis code-spec 必须保持同一信任口径：下层模型/rootlet/tool/search 输出是未收束材料；上层中枢 agent 的综合、裁决和交接判断是地下集群的主执行路径；工程边界只守协议、权限、预算、hard constraint、状态机、谱系、脱敏、文件和治理边界，禁止替 agent 思考。
