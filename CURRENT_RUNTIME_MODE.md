...
- Ordinary 模型上下文中的历史消息、模型压缩摘要和已选 Skill 正文不再先按固定字符数二次裁剪；容量由冻结的模型 token budget 与已有 context compaction 管理。模型请求计量覆盖正文、工具参数、协议扩展和附件元数据，并为无法本地精确计量的二进制附件预留保守预算，provider 仍是最终计量边界；压缩不得拆开 assistant tool call 与对应 tool result。下一轮会从紧邻上一轮 Ordinary run 的同一工具事件 reducer 恢复最多 24 组有界 input/output/error/截断标记与事件引用，作为模型可见的连续性事实；失败或取消 run 的可见进度与停止原因也会进入连续性上下文。这里不恢复 raw provider 协议对象，不把 `tool-output://` 的 live-only 完整内容伪装成可跨进程读取，也不建设长期记忆；事实已截断、引用已失效或时效性重要时，由模型再次调用原工具
